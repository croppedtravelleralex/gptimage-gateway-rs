...
| MVP 本地 | Panda `:8013` | 夹具差分绿 |
| Panda MVP | 全量开发 | **MVP 节签字**：`self=0` |
...
样本建议：生文 n≥5，生图 n≥5；固定 pin；模型按现网（生图例 `gpt-image-2`）。
...
| M-I1 | generations 成功 | 200 + b64_len>1000 | ok | P50 ~30–120s | self=0 |
| M-I2 | SSE ready=cid | 无 post_ready=15s | ok | — | 硬 |
...
|------|------------------------------|----------------|
| text 成功/总数 | ≥ Python − 0（剔 upstream） | |
| image 成功/总数 | ≥ Python − 0（剔 upstream/gate） | |
| self 计数 | **0** | |
| 假成功空 data | **0** | |
| text P50 | ≤ Python × 1.10 | |
| image P50 / P95 | ≤ ×1.15 / ×1.20 | |
| runlog 路径 | | `data/runlogs/rust-canary-*` |

**MVP 出门签字**：日期 ____ 证据路径 ____ `self=0` □
