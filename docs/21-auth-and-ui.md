...
> **⚠️ 本文描述的是目标状态，不是线上状态。**
>
> 相关代码存在于本地工作树，但**未提交进 git，也从未部署**。panda `:8013` 实测
> `/api/auth/me`、`/api/auth/login`、`/api/admin/users`、`/api/backend/capabilities`、
> `/v1/images/edits` **全部 404** —— 运行中的二进制是 commit `7c34159`（2026-07-21 编译），
> 只有 456 行 `main.rs` 和 7 条路由，不含本文任何内容。
>
> 恢复条件见 `../plan.md` §6.1 与 `17-operator-guide.md`「让文档变成真的」。
